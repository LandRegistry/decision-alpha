...
class DecisionServiceTestCase(unittest.TestCase):
    ...
    def test_perform_action_returns_200_and_send_to_casework_response_when_data_present_and_check_is_true(self):
        with self.app.test_request_context():
            ...
            self.assertEquals(json.loads(action_response.data), {"url": "casework_url",
                                                                 "signed-token": "1234",
                                                                 "transaction-id": "ABCDEFG"})
    ...
    def test_perform_action_returns_200_and_send_to_check_when_country_code_is_not_gb(self):
        with self.app.test_request_context():
            ...
            self.assertEquals(action_response.status_code, 200)
            self.assertEquals(json.loads(action_response.data), {"url": "check_url",
                                                                 "signed-token": "1234",
                                                                 "transaction-id": "ABCDEFG"})
    ...
    def test_perform_action_returns_400_when_data_is_missing(self):
        with self.app.test_request_context():
            ...
            action_response = decision_service.perform_action(bad_data, 'casework_url', 'check_url')
            self.assertEquals(action_response.status_code, 400)
            self.assertEquals(json.loads(action_response.data), {'error_message': 'Missing country code'})
    ...
    def test_perform_action_returns_400_when_action_is_not_valid(self):
        with self.app.test_request_context():
            ...
            action_response = decision_service.perform_action(bad_data, 'casework_url', 'check_url')
            self.assertEquals(action_response.status_code, 400)
            self.assertEquals(json.loads(action_response.data), {'error_message': 'Action not found'})
    ...
    def test_make_error_response_returns_bad_request_response_with_given_message(self):
        with self.app.test_request_context():
            ...
            self.assertEquals(json.loads(err.data), {'error_message': msg})
